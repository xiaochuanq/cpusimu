...
public:
 Exp4Processor(int nArchReg, int nPhyReg, uint32_t nRobDepth, uint32_t nWidth, branch_predictor* ppred) : 
		Exp3Processor(nArchReg, nPhyReg, nRobDepth, nWidth),
		  m_nFetchReady(0), m_pPred(ppred) {
	};
		~Exp4Processor(){delete m_pPred;}
	void issue();
	void fetch(istream&);
private:
	int m_nFetchReady;
	branch_predictor* m_pPred;
};
