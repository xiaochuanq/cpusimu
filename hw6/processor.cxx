...
void Exp4Processor::issue() {
	...
	for (ROB::iterator rit = m_rob.begin(); rit != m_rob.end(); ++rit) {
		if (!rit->HasIssued() && isReady(*rit)) {
			...
			if (rit->IsMisPredicted()) { // stall fetching if mis-predicted
...
void Exp4Processor::fetch(istream& ins) {
	...
	for (unsigned int i = 0; i < width(); ++i) {
		if (m_nFetchReady) {
			...
			renameInstruction(*pinstrn);
			unReady(*pinstrn);
			if (pinstrn->IsCondBranch()) {
				bool pred = m_predictor.predict(pinstrn->PC());
				if (pred != pinstrn->DefactoBranch()){
					pinstrn->markMisprediction();
